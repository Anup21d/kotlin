...
abstract class AbstractWriteSignatureTest : CodegenTestCase() {
    ...
    override fun doMultiFileTest(wholeFile: File, files: MutableList<TestFile>, javaFilesDir: File?) {
        ...
        try {
            val expectations = parseExpectations(wholeFile)
            expectations.check()
        }
        catch (e: Throwable) {
    ...
    private inner class ClassExpectationsSuite(val className: String) {
        ...
        private fun processPackageParts(checker: Checker, relativeClassName: String) {
            ...
            val partPrefix = relativeClassName.replace(".class", "\$")
        ...
        private fun processClassFile(checker: Checker, classFile: ByteArray) {
